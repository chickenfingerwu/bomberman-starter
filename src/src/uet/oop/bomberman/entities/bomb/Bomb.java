...
public class Bomb extends AnimatedEntitiy {
	...
	public void render(Screen screen) {
		if(_exploded) {
			_sprite =  Sprite.movingSprite(Sprite.bomb_exploded, Sprite.bomb_exploded1, Sprite.bomb_exploded2,_animate, 17);
			renderFlames(screen);
		} else
	...
	protected void explode() {
		...
		if(e instanceof DestroyableTile){
			Entity s = ((LayeredEntity) e).getTopEntity();
			if(s instanceof Brick){
				((Brick)s).destroy();
			}
		}
		if(b.getXTile() == _x && b.getYTile() == _y){
			if(Game.isFlamePassThrough() && _timeAfter == 0){
